...
export default function generateSeedData() {
	...
	// Assurer une salle de chaque catégorie
	for (let categoryIndex = 0; categoryIndex < 5; categoryIndex++) {
		const categoryRoom = allNonEmptyRooms.find(
			(room) => room.type === categoryIndex
		);
		if (categoryRoom) {
			selectedRooms.push(categoryRoom);
			allNonEmptyRooms = allNonEmptyRooms.filter(
				(room) => room !== categoryRoom
			);
		}
	}
	...
	for (
		let i = selectedRooms.length;
		i < numberOfFilledRooms && allNonEmptyRooms.length > 0;
		i++
	) {
		const randomIndex = Math.floor(Math.random() * allNonEmptyRooms.length);
		selectedRooms.push(allNonEmptyRooms[randomIndex]);
		allNonEmptyRooms.splice(randomIndex, 1);
	}

	// Ajouter les salles vides
	for (let i = 0; i < numberOfEmptyRooms; i++) {
		selectedRooms.push({ empty: true, type: 'empty', number: i });
	}

	// Mélanger toutes les salles sélectionnées
