...
export default function Flashlight({
	...
	const flashlightParts = useMemo(
		() => [{ name: 'lightTexture', label: 'Light Texture' }],
		[]
	);

	const { loadedItems } = useProgressiveLoad(flashlightParts, 'Flashlight');

	const visibleParts = useMemo(() => {
		return flashlightParts.reduce((acc, part) => {
			acc[part.name] = loadedItems.some((item) => item.name === part.name);
			return acc;
		}, {});
	}, [loadedItems, flashlightParts]);

	useEffect(() => {
		if (visibleParts.lightTexture) {
