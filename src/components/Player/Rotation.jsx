...
export default function Rotation({
	...
	const reset = useCallback(() => {
		if (isMobile) {
			playerPosition.current.set(0, floor, 0);
		} else {
		...
		pitch.current = 0;
	}, [camera, isMobile, playerPosition, playerVelocity]);
	...
	useFrame((state) => {
		if ((deviceMode === 'gamepad' || isMobile) && monsterState !== 'run') {
			...
			// Update rightStickRef from gamepad controls
			if (gamepadControls.rightStickX !== undefined) {
				rightStickRef.current.x = gamepadControls.rightStickX;
				rightStickRef.current.y = gamepadControls.rightStickY;
			}
