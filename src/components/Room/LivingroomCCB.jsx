...
export default function LivingroomCCB() {
	const { scene, nodes } = useGLTF('/models/room/livingroomCCB.glb');
	...
	const textureParts = [
		{
			name: 'baked',
			label: 'Base Textures',
			texture: useKTX2('/textures/livingroom/CCB/baked_livingroom_etc1s.ktx2'),
			type: 'map',
			uvChannel: 0,
		},
		{
			...
			texture: useKTX2(
				'/textures/livingroom/CCB/roughness_livingroom_etc1s.ktx2'
			),
		...
		},
		{
			name: 'light',
			label: 'Lighting',
			texture: useKTX2('/textures/livingroom/CCB/light_livingroom_uastc.ktx2'),
			type: 'lightMap',
			uvChannel: 2,
		},
	];
	...
	return (
		<>
			...
			<mesh position={[4.255, 1.648, -0.17]} rotation={[0, -Math.PI / 2, 0]}>
				<planeGeometry args={[1.37, 0.9]} />
				<meshStandardMaterial>
					<primitive object={useTexture('/images/botkz.webp')} attach="map" />
				</meshStandardMaterial>
			</mesh>

			<mesh position={[3.4, 1.7, 4.775]} rotation={[0, -Math.PI / 2, 0]}>
			...
			<mesh position={[3.4, 1, 4.775]} rotation={[0, -Math.PI / 2, 0]}>
